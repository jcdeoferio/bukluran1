class NotifierController < ApplicationController
	...
	def get_confirmation
	   ...
	   @org.update_attribute('confirmation_sent', true)
	   redirect_to "/organizations/show_forms/#{@org.id}"
	end
	...
	def conkey
	 @key = @org.name.crypt "#{@member.id}pDeNMirfmconFrimmd123nf3fh321dfrgFMJSD#{Time.now.sec}"
	 @key.gsub!("/", "!")
	 return @key
	end
